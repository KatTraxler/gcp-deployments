...
resource "google_service_account" "task_service_account" {
  project = var.project_id
  account_id   = "task-service-account"
  display_name = "Service Account used for containerized tasks"
}
resource "google_project_iam_binding" "project_viewer" {
  ...
  members = [
    google_service_account.task_service_account.member
  ]
}